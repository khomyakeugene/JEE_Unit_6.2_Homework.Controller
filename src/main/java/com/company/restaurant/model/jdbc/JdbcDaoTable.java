 ...
 */
public class JdbcDaoTable<T> extends JdbcDao<T> {
    protected static final String SQL_ALL_FIELD_BY_FIELD_VALUE = "SELECT * FROM %s WHERE (%s = ?)";
    protected static final String SQL_ALL_FIELD_OF_ALL_RECORDS = "SELECT * FROM %s";
    ...
    protected ResultSet findAll() {
        ...
        try(Connection connection = dataSource.getConnection();
            PreparedStatement preparedStatement = connection.prepareStatement(
                    String.format(SQL_ALL_FIELD_OF_ALL_RECORDS, tableName))) {
            resultSet = preparedStatement.executeQuery();
        } catch (SQLException e) {
            databaseConnectError(e);
        }
